...
class AptsWhereShareholderWantsOnlyMinutesHardCopyReportWriter
{
    ...
    public function createSpreadsheetAptsShareholderWantsMinutesHardCopy() {
       ...
       $phpExcelObject = $this->getPhpExcelObjectAndSetHeadings(self::APTS_WHERE_SHAREHOLDER_WANTS_MINUTES_HARD_COPY_HEADER_ARRAY, $title, $description, $category);
            ...
            if (!is_null($phpExcelObject) and $phpExcelObject instanceof \PHPExcel) {
                ...
                $prevBuildingId = "";
                $prevAptName = "";
                $prevAptWithNoEmailAddressesRow = null;
                $lastRowWrittenFlag = false;
                foreach ($apartmentsWithNoEmailAddresses as $apartmentWithNoEmailAddressRow) {
                    ...
                    if ($prevAptName !== "" and $prevAptName !== $apartmentWithNoEmailAddressRow[self::APTS_WHERE_SHAREHOLDER_WANTS_MINUTES_HARD_COPY_COL_NAMES[4]]) {
                        $lastRowWrittenFlag = true;
                        for ($i = 0; $i < $colLimit; $i++) {
                            ...
                            $phpExcelObject->getActiveSheet()
                                ->setCellValue($cellId, $prevAptWithNoEmailAddressesRow[self::APTS_WHERE_SHAREHOLDER_WANTS_MINUTES_HARD_COPY_COL_NAMES[$i]]);
                            // }
                        }
                     ...
                     $prevAptName    =  $apartmentWithNoEmailAddressRow[self::APTS_WHERE_SHAREHOLDER_WANTS_MINUTES_HARD_COPY_COL_NAMES[4]];
                    $prevAptWithNoEmailAddressesRow = $apartmentWithNoEmailAddressRow;

                // }

                }

                if ($lastRowWrittenFlag == false ) {
                    for ($i = 0; $i < $colLimit; $i++) {
                        $currentLetter = PHPExcel_Cell::stringFromColumnIndex($i);
                        $cellId = $currentLetter . $rowCtr;
                        $phpExcelObject->getActiveSheet()
     ...
     *
     *  The status code of ']' means the shareholder only wants to receive hard copies
